...
eigenvector_result_path = r'C:\\Users\\KJG\\Desktop\\eigenvector_result.csv'  # 업로드된 고유벡터 중심성 결과
...
# 각 단어의 TF-IDF 점수를 평균하여 상위 20개 키워드 추출
...
# 상위 20개 TF-IDF 키워드 추출 후 Min-Max 스케일링 적용
top_20_terms = sorted_terms[:20]
top_20_keywords = pd.DataFrame(top_20_terms, columns=["TF-IDF Keyword", "TF-IDF Score"])

# 고유벡터 중심성 결과 로드
eigen_df = pd.read_csv(eigenvector_result_path)

# 고유벡터 중심성 상위 키워드와 겹치지 않는 TF-IDF에만 존재하는 키워드를 필터링하고 Min-Max 스케일링 적용
def get_tfidf_only_keywords(eigen_keywords, tfidf_df):
    return tfidf_df[~tfidf_df['TF-IDF Keyword'].isin(eigen_keywords)]

tfidf_only_keywords = get_tfidf_only_keywords(eigen_df['eigen_keyword'], top_20_keywords)
tfidf_only_keywords["Scaled TF-IDF Score"] = minmax_scale(tfidf_only_keywords["TF-IDF Score"], feature_range=(0, 1))

# 고유벡터와 TF-IDF에만 존재하는 키워드의 조합 점수 계산 및 별도 CSV 저장
combined_keywords = []
if not eigen_df.empty:
    for _, eigen_row in eigen_df.iterrows():
        for _, tfidf_row in tfidf_only_keywords.iterrows():
    ...
    combined_output_path = r'C:\\Users\\KJG\\Desktop\\combined_model_1_keywords_scores.csv'
